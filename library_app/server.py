...
class MakeReservationHandler(BaseHandler):
    async def post(self):
        ...
        query = (
            "INSERT INTO reservations (reservation_id, book_id, customer_id, reservation_date) "
            "VALUES (%s, %s, %s, %s);"
        )
        self.cassandra_session.execute(
            query, (reservation_id, book_id, customer_id, reservation_date)
        )
...
class UpdateReservationHandler(BaseHandler):
    async def post(self):
        ...
        self.cassandra_session.execute(query, (int(reservation_date), book_id, customer_id))
...
def make_app(cassandra_session):
...
if __name__ == "__main__":
    cluster = Cluster(["127.0.0.1"], port=9042)  # Replace with your Cassandra cluster nodes
